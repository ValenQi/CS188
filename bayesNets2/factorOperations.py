...
def normalize(factor):
    ...
    if totalProb == 0:
        ...
        for key in variableDomainsDict:
            if len(variableDomainsDict[key]) == 1:
                if key in unconditionedVariables:
                    unconditionedVariables.remove(key)
                    conditionedVariables.add(key)
        newFactor = Factor(unconditionedVariables, conditionedVariables, variableDomainsDict)
        for assignment in assignments:
            newFactor.setProbability(assignment, factor.getProbability(assignment)/totalProb)
        return newFactor
