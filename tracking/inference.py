...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        total = self.total()
        bound = 0.0
        sample = random.uniform(0.0, total)
    ...
    def elapseTime(self, gameState):
        ...
        for position in self.allPositions:
            ...
            for key, value in newPosDist.items():
                temp[key] += value * self.beliefs[position]
        self.beliefs = temp
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        if beliefs.total() == 0:
            ...
            beliefs.normalize()
            self.particles = [beliefs.sample() for i in range(0, self.numParticles)]
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            self.particles.append(allPositions[i % len(allPositions)])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                beliefs[particle] *= self.getObservationProb(observation[i], pacmanPosition, particle[i], self.getJailPosition(i))
        if beliefs.total() == 0:
            self.initializeUniformly(gameState)           
        else:
            beliefs.normalize()
